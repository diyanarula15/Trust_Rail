...
| Telegram channel adapter | **Real, fully live-capable** | `channels/telegram.py` + `scripts/telegram_poll.py` (real long-poll loop, no public URL needed). Ships with no bot token configured, so tested via `scripts/telegram_sim.py`, which drives the same `dispatch_update()` the real poller uses — verdicts are always real, only the Telegram transport is simulated |
