...
| Telegram channel adapter | **Real, fully live-capable, dual transport** | `channels/telegram.py` supports both `scripts/telegram_poll.py` (long-poll, no public URL needed) and `api/webhooks_telegram.py` (push, mirrors WhatsApp's model, fail-closed secret-token check) — mutually exclusive at the Telegram API level, both drive the same `dispatch_update()`. Ships with no bot token configured, so tested via `scripts/telegram_sim.py` (in-process) and `scripts/telegram_webhook_sim.py` (real HTTP to the webhook route) — verdicts are always real, only the Telegram transport is simulated |
| Email channel adapter | **Real, fully live-capable** | `channels/email_channel.py` + `scripts/email_poll.py` (real IMAP polling + SMTP reply, no public URL needed). Ships with no mailbox configured, so tested via `scripts/email_sim.py` against the real `fixtures/eml_samples/*.eml` fixtures |
| WhatsApp channel adapter | **Built, needs credentials** | `channels/whatsapp.py` + `api/webhooks_whatsapp.py`: HMAC-SHA256 webhook signature verification, media download, and replies rendered from the same `render.py` payload the web UI uses. Flag-gated off (`CHANNEL_WHATSAPP_ENABLED`); inert without real Meta credentials. Unlike Telegram/email it has no polling mode, so it can't go fully live without a Meta Business account *and* a public tunnel. Tested via `scripts/whatsapp_sim.py`, which POSTs a byte-accurate fake payload over real HTTP and can substitute a local file for a real Meta media download; see `docs/SETUP_WHATSAPP.md` |
