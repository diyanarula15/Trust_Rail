...
def dispatch_update(update: dict[str, Any], db: Session) -> dict[str, Any] | None:
    """Handles one Telegram update end-to-end: ingest -> real verification
    pipeline -> formatted reply -> real-or-logged send. Returns the sent (or
    sim-logged) payload, or None if the update carried no message to act on.

    Shared by the real long-poll loop (scripts/telegram_poll.py) and the
    local simulator (scripts/telegram_sim.py) — both call this exact
    function, so the sim path exercises real handling logic, not a mock.
    """
