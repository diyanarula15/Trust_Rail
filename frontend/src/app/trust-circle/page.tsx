...
import { TrustCircleSimulation } from "@/components/TrustCircleSimulation";
...
export default function TrustCirclePage() {
  const [mode, setMode] = useState<"app" | "simulation">("app");
  ...
  return (
    <Page>
      ...
      <Reveal delay={40} className="mb-8 inline-flex rounded-full border border-hairline bg-card p-1">
        <button
          type="button"
          onClick={() => setMode("app")}
          className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
            mode === "app" ? "bg-ink text-paper" : "text-info hover:text-ink"
          }`}
        >
          App
        </button>
        <button
          ...
          className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
            mode === "simulation" ? "bg-ink text-paper" : "text-info hover:text-ink"
          }`}
        >
          Simulation
        </button>
